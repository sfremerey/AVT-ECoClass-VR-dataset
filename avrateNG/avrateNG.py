...
@route('/postquestionnaire', method='POST')
@auth_basic(check_credentials)
def postquestionnaire_save(db, config):
    """
    saves demographic info into sqlite3 table,
    all user information (user_id is key in tables) are stored as JSON string
    """
    ...
    db.execute('CREATE TABLE IF NOT EXISTS post_info (user_ID, info_json TEXT);')
    db.execute('INSERT INTO post_info VALUES (?,?);',(user_id, json.dumps(dict(request.forms))))
    db.commit()
    redirect('/done')

@route('/done')  # Finish screen
@auth_basic(check_credentials)
def done(config):
    """
    will be shown after test was completly done
    """
    return template(config["template_folder"] + "/done.tpl", title="AVRateNG")
